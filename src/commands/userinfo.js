...
exports.run = async (client, message, args) => {
  try {
    ...
    if (!user) {
      message.channel.send(`${client.users.cache.get(searchUserId).username} is not registered.`);
      return;
    }
    message.channel.send(`User information for ${client.users.cache.get(searchUserId).username}:\`\`\`BNet ID: ${user.bnet_id}\nTimezone: ${user.timezone}\nTwitch: ${(user.twitch ? user.twitch : 'Not set')}\nNotifications: ${(user.newEventNotification ? 'On' : 'Off')}\`\`\``);
  } catch (err) {
    throw new Error(err);
  }
};